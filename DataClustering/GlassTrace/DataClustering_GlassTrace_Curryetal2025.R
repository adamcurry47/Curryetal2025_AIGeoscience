...
## read in data
inputdata <- read.csv("CentralSanJuanMinChemCompilation_GlassTrace.csv")
...
data1 <- inputdata[,c("Li_ppm","Rb_ppm","Sr_ppm","Y_ppm","Zr_ppm","Cs_ppm","Ba_ppm","Ce_ppm","Nd_ppm","Hf_ppm","Th_ppm","U_ppm","Eruption")]  ## makes sure the names exactly math those in your column headers
...
elements<- c("Li_ppm","Rb_ppm","Sr_ppm","Y_ppm","Zr_ppm","Cs_ppm","Ba_ppm","Ce_ppm","Nd_ppm","Hf_ppm","Th_ppm","U_ppm") ## makes sure the names exactly math those in your column headers
...
##normalize
data1_norm<- as.data.frame(lapply(data1_ilr[,c("V1","V2","V3","V4","V5","V6","V7","V8","V9","V10","V11")], normalise)) ##the number of vectors here is 1 less than the number of elements chosen above
...
write.csv(x = data1_fin, file = "glasstrace_medianSD_4clust_forRandTrees.csv")
...
write.csv(x = data1_clusters, file = "glasstrace_medianSD_4clust_forRandTrees_wClusters.csv")
...
par(mfrow=c(1,1))
pie(c(Cl1_perc,Cl2_perc,Cl3_perc,Cl4_perc),labels = c("C1 - 25%","C2 - 22%","C3 - 28%","C4 - 25%"), 
    col=c("black","red","green","blue"),main="Glass Trace Clusters - All")
...
pie(c(SM1,SM2,SM3,SM4),labels = c("SM1-76","SM2-24","SM3-0","SM4-0"), 
    col=c("black","red","green","blue"),main="Glass Trace Clusters - SM")
...
pie(c(NM1,NM2,NM3,NM4),labels = c("NM1-20","NM2-0","NM3-78","NM4-2"), 
    col=c("black","red","green","blue"),main="Glass Trace Clusters - NM")
...
pie(c(CC1,CC2,CC3,CC4),labels = c("CC1-0","CC2-100","CC3-0","CC4-0"), 
    col=c("black","red","green","blue"),main="Glass Trace Clusters - CC")
...
pie(c(RC1,RC2,RC3,RC4),labels = c("RC1-0","RC2-0","RC3-0","RC4-100"), 
    col=c("black","red","green","blue"),main="Glass Trace Clusters - RC")
