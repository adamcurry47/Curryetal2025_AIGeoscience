...
## read in data
inputdata <- read.csv("CentralSanJuanMinChemCompilation_GlassMajor.csv")
...
data1 <- inputdata[,c("SiO2_wt","Al2O3_wt","FeO_wt","CaO_wt","K2O_wt","Na2O_wt","Eruption")]  ## makes sure the names exactly math those in your column headers
...
elements<- c("SiO2_wt","Al2O3_wt","FeO_wt","CaO_wt","K2O_wt","Na2O_wt") ## makes sure the names exactly math those in your column headers
...
##normalize
data1_norm<- as.data.frame(lapply(data1_ilr[,c("V1","V2","V3","V4","V5")], normalise)) ##the number of vectors here is 1 less than the number of elements chosen above
...
write.csv(x = data1_fin, file = "glassmajor_medianSD_5clust_forRandTrees.csv")
...
write.csv(x = data1_clusters, file = "glassmajor_medianSD_5clust_forRandTrees_wClusters.csv")
...
par(mfrow=c(1,1))
pie(c(Cl1_perc,Cl2_perc,Cl3_perc,Cl4_perc,Cl5_perc),labels = c("C1 - 26%","C2 - 6%","C3 - 13%","C4 - 18%","C5 - 37%"), 
    col=c("black","red","green","blue","gray"),main="Glass Major Clusters - All")
...
pie(c(SM1,SM2,SM3,SM4,SM5),labels = c("SM1 - 72%","SM2 - 15%","SM3 - 12%","SM4 - 1%","SM5 -0%"), 
    col=c("black","red","green","blue","gray"),main="Glass Major Clusters - SM")
...
pie(c(NM1,NM2,NM3,NM4,NM5),labels = c("NM1 - 0%","NM2 - 8%","NM3 - 86%","NM4 - 7%","NM5 - 0%"), 
    col=c("black","red","green","blue","gray"),main="Glass Major Clusters - NM")
...
pie(c(CC1,CC2,CC3,CC4,CC5),labels = c("CC1 - 2%","CC2 - 1%","CC3 - 0%","CC4 - 0%","CC5 - 97%"), 
    col=c("black","red","green","blue","gray"),main="Glass Major Clusters - CC")
...
pie(c(RC1,RC2,RC3,RC4,RC5),labels = c("RC1 - 12%","RC2 - 0%","RC3 - 1%","RC4 - 77%","RC5 - 10%"), 
    col=c("black","red","green","blue","gray"),main="Glass Major Clusters - RC")
