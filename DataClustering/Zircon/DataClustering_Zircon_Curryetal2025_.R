...
## read in data
inputdata <- read.csv("CentralSanJuanMinChemCompilation_Zircon.csv")

##Choose which dataset you want to cluster. For this paper, I chose to cluster only data from the publications Curry et al. (2021a) and Curry et al. (2021b)
inputdata <- inputdata[c(which(inputdata$DataSource == "Curryetal2021b")),] 
##remove Ti data that has NA (i.e., no data)
inputdata <- inputdata[-c(which(is.na(inputdata$Ti_ppm))),] 

###Choose specific elements to use in clustering 
data1 <- inputdata[,c("Ti_ppm","Y_ppm","Zr_ppm","Ce_ppm","Gd_ppm","Dy_ppm","Ho_ppm","Er_ppm","Tm_ppm","Yb_ppm","Lu_ppm","Hf_ppm","Th_ppm","U_ppm","Eruption")]
## makes sure the names exactly math those in your column headers
...
elements<- c("Ti_ppm","Y_ppm","Zr_ppm","Ce_ppm","Gd_ppm","Dy_ppm","Ho_ppm","Er_ppm","Tm_ppm","Yb_ppm","Lu_ppm","Hf_ppm","Th_ppm","U_ppm") ## makes sure the names exactly math those in your column headers
...
##normalize
data1_norm<- as.data.frame(lapply(data1_ilr[,c("V1","V2","V3","V4","V5","V6","V7","V8","V9","V10","V11","V12","V13")], normalise))
...
write.csv(x = data1_fin, file = "zrn_medianSD_6clust_forRandTrees.csv")
...
write.csv(x = data1_clusters, file = "zrn_medianSD_6clust_forRandTrees_wClusters.csv")
...
par(mfrow=c(1,1))
pie(c(Cl1_perc,Cl2_perc,Cl3_perc,Cl4_perc,Cl5_perc,Cl6_perc),labels = c("C1 - 16%","C2 - 14%","C3 - 42%","C4 - 18%",
                                                                        "C5 - 6%","C6 - 4%"), col=c("black","red","green","blue","gray","deepskyblue"),main="Zircon Clusters - All")
...
pie(c(SM1,SM2,SM3,SM4,SM5,SM6),labels = c("SM1-16","SM2-34","SM3-34","SM4-14","SM5-1","SM6-1"), 
    col=c("black","red","green","blue","gray","deepskyblue"),main="Zircon Clusters - SM")
...
pie(c(NM1,NM2,NM3,NM4,NM5,NM6),labels = c("NM1-3","NM2-0","NM3-65","NM4-10","NM5-19","NM6-3"), 
    col=c("black","red","green","blue","gray","deepskyblue"),main="Zircon Clusters - NM")
...
pie(c(CC1,CC2,CC3,CC4,CC5,CC6),labels = c("CC1-9","CC2-9","CC3-40","CC4-34","CC5-6","CC6-2"), 
    col=c("black","red","green","blue","gray","deepskyblue"),main="Zircon Clusters - CC")
...
pie(c(RC1,RC2,RC3,RC4,RC5,RC6),labels = c("RC1-42","RC2-7","RC3-38","RC4-0","RC5-3","RC6-10"), 
    col=c("black","red","green","blue","gray","deepskyblue"),main="Zircon Clusters - RC")
